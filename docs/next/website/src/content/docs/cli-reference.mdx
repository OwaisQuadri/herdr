...
By default, `machine add` uses the remote default session. Add `--remote-session <name>` only for a named session. Saved machines default to `local` keybindings; use `machine keybindings <profile-id> server` to use a saved machine's server keybindings and advertised custom commands. `machine list` accepts optional `--json` for scripts; each row includes the `keybindings` preference. See [Connecting machines](/docs/connecting-machines/) for the full setup and connection guide.
