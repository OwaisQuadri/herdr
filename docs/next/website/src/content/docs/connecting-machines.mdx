...
herdr machine keybindings <profile-id> server
...
Saved machines use local keybindings by default. To use one machine's server keybindings and its advertised custom commands, set `herdr machine keybindings <profile-id> server`. Set it back to `local` to restore the default. Server-mode custom commands stay opaque and are invoked on that server; local custom shell commands are never sent to it.
...
The UI uses the client's local theme, sidebar settings, and keybindings by default. A saved machine set to `server` keybindings instead uses that endpoint's built-in keybindings and advertised custom commands. Custom commands and plugins advertised by the selected server still run there. Herdr does not copy local command plugins, configuration, executables, or secrets onto SSH hosts. Missing remote commands fail visibly. Use the UI's `reload config` action after editing client settings; see [Configuration](/docs/configuration/#reload-config).
...
Saved profiles contain only an opaque ID, label, SSH target, explicit remote session, enabled state, and `local` or `server` keybinding preference. Herdr does not store passwords, private keys, agent tickets, or SSH control sockets in the catalog. Authentication stays with OpenSSH.
